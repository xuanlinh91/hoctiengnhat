...
function doUpload() {
    document.getElementById('progress-group').removeAttribute('style');
...
function uploadFile(file, index) {
    ...
    var Uploaded_link = document.getElementById('thumb_link');
    //Div.Progress
    ...
    http.upload.addEventListener('progress', function(event) {
        ...
        //Chờ dữ liệu trả về
        if (fileProgress == 100) {
            ProgressBar.style.background = 'url("/hoctiengnhat/images/progressbar.gif")';
        }
    ...
    http.open('POST', '/hoctiengnhat/upload.php', true);
    http.send(data);
        ...
        if (http.readyState == 4 && http.status == 200) {
            ...
            try { //Bẫy lỗi JSON
                ...
                if (server.status) {
                    ...
                    var file_path = document.getElementById('myfile').value;
                    var file_name = file_path.split("\\").pop(-1);
                    Uploaded_link.value = file_name; //Thông báo
                    document.getElementById('thumb_img').setAttribute('src', '/hoctiengnhat/uploads/' + file_name);
                    document.getElementById('thumb_img').removeAttribute('style');
                    document.getElementById('del_thumb').removeAttribute('style');
                    ...
                    document.getElementById('progress-group').setAttribute('style', 'display:none');
                } else {
